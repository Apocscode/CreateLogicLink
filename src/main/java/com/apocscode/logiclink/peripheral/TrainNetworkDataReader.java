    ...
    private static void analyzeDiagnostics(CompoundTag mapData) {
        ...
        for (Map.Entry<Integer, List<int[]>> entry : adjacency.entrySet()) {
            ...
            for (int bi = 0; bi < branchDirs.size(); bi++) {
                ...
                // This handles crossing geometry where internal edges are tiny (1.5b).
                ...
                if (dist < 3.0f) {
                    ...
                    if (neighborBranches != null && neighborBranches.size() == 2) {
                        for (int[] nb : neighborBranches) {
                            if (nb[0] == jId) continue;
                            int contId = nb[0];
                            if (contId >= nodes.size()) continue;
                            CompoundTag contNode = nodes.getCompound(contId);
                            float cnx = contNode.getFloat("x"), cny = contNode.getFloat("y"), cnz = contNode.getFloat("z");
                            float cdx2 = cnx - nx, cdy2 = cny - ny, cdz2 = cnz - nz;
                            float contDist = (float) Math.sqrt(cdx2 * cdx2 + cdy2 * cdy2 + cdz2 * cdz2);
        ...
        // Chain signals look ahead for a regular signal as a stopping point.
        // With chain on both sides, there's no stopping point in either direction.
        // This is a guaranteed NO_PATH cause.
        for (int i = 0; i < signals.size(); i++) {
            CompoundTag sig = signals.getCompound(i);
            ...
            if (typeF.equals("cross_signal") && typeB.equals("cross_signal")) {
                ...
                diag.putFloat("y", sy);
                diag.putFloat("z", sz);
                diag.putString("desc", "Signal has chain on BOTH sides — no stopping point in either direction. "
                        + "Use wrench to set one side to regular, or remove this signal. "
                        + "Chain signals need a regular signal downstream as a terminator.");
                diagnostics.add(diag);
        ...
        // Check 12 removed: Crossings DO need chain signals for collision prevention.
        // The previous Check 12 incorrectly told users to remove crossing signals.
